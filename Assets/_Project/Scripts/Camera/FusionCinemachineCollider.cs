...
namespace Cinemachine
{
    ...
    public class FusionCinemachineCollider : CinemachineExtension
    {
        ...
        {
            FusionRuntimeUtility.DestroyScratchCollider();
            base.OnDestroy();
        }
            ...
            Vector3 cameraPos, Vector3 lookAtPos, int layerMask, ref RaycastHit hitInfo)
        {
            ...
            if (targetDistance > Epsilon)
            {
                ...
                else
                {
                    ...
                    {
                        if (FusionRuntimeUtility.RaycastIgnoreTag(
                            runner, ray, out hitInfo, rayLength, layerMask, m_IgnoreTag))
                        {
            ...
            ref VcamExtraState extra)
        {
            ...
            if (FusionRuntimeUtility.RaycastIgnoreTag(runner, ray, out var hitInfo, distance,
            ...
            if (d < Epsilon || FusionRuntimeUtility.RaycastIgnoreTag(
                    runner, new Ray(lookAtPos, dir), out _, d - k_PrecisionSlush,
                    m_CollideAgainst & ~m_TransparentLayers, m_IgnoreTag))
                return currentPos;
            ...
            if (distance > Epsilon)
            {
                if (!FusionRuntimeUtility.RaycastIgnoreTag(runner, ray, out hitInfo, distance,
                        m_CollideAgainst & ~m_TransparentLayers, m_IgnoreTag))
                {
            ...
            Vector3 pos, Vector3 pushDir, RaycastHit obstacle, ref Vector3 outDir)
        {
            ...
            int numFound = runner.GetPhysicsScene().SphereCast(
        ...
        Vector3 RespectCameraRadius(Vector3 cameraPos, Vector3 lookAtPos)
        {
            ...
            RaycastHit hitInfo;
            int numObstacles = runner.GetPhysicsScene().OverlapSphere(
                ...
                && distance > m_MinimumDistanceFromTarget + Epsilon)
            {
                ...
                if (FusionRuntimeUtility.RaycastIgnoreTag(runner, new Ray(targetPos, dir), 
            ...
            {
                var scratchCollider = FusionRuntimeUtility.GetScratchCollider();
        ...
        bool CheckForTargetObstructions(CameraState state)
        {
            if (state.HasLookAt)
            {
                ...
                if (FusionRuntimeUtility.RaycastIgnoreTag(runner, ray, out _,
