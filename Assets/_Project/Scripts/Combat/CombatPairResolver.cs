        ...
        private void CollisionPairResolver()
        {
            foreach (var pair in pairFinder.collisionPairs)
            {
                ...
                if (overlapped)
                {
                    direction.y = 0;
                    direction.Normalize();
                    pair.Key.Item1.GetComponent<FighterPhysicsManager>().Move(pair.Key.Item1.transform.position + (direction * (distance / 2.0f) * collisionSolvePercentage));//.kCC.Motor.MoveCharacter(pair.Key.Item1.transform.position + (direction * (distance/2.0f) * collisionSolvePercentage));
                    pair.Key.Item2.GetComponent<FighterPhysicsManager>().Move(pair.Key.Item2.transform.position - (direction * (distance / 2.0f) * collisionSolvePercentage));//.kCC.Motor.MoveCharacter(pair.Key.Item2.transform.position - (direction * (distance/2.0f) * collisionSolvePercentage));
                }
            }
